...
export default function DashboardPage() {
  ...
  const startSimulationRecording = (sentence: string) => {
    ...
    if (!normalizedSentence) {
      setFinalTranscript("No simulation sentence configured.");
      stopSimulation();
    ...
    const streamNextWord = () => {
      index += 1;
      setFinalTranscript(words.slice(0, index).join(" "));
      ...
      const nextDelay = 500;
      simulationTimeoutRef.current = setTimeout(streamNextWord, nextDelay);
    };

    const initialDelay = 800;
    simulationTimeoutRef.current = setTimeout(streamNextWord, initialDelay);
  };
  ...
  */
  ...
  const startRecording = async () => {
    const simulationSettings = readSimulationSettings();
    const shouldSimulate =
      user?.email === VOXSIGN_SIMULATION_EMAIL && simulationSettings.simulate;

    if (shouldSimulate) {
      startSimulationRecording(simulationSettings.sentence);
      return;
    }
        ...
        <div className="flex min-h-[360px] w-full items-center justify-center rounded-[28px] bg-gradient-to-b from-[#F6FAFE] to-white px-6 py-10">
          {isSimulating && isRecording ? (
            <div className="flex w-full flex-col items-center text-center">
              ...
              <div className="flex min-h-[220px] w-full items-center justify-center rounded-[30px] border border-[#DDEAFF] bg-[#FBFDFF] p-8 shadow-inner">
        ...
        <div className="relative overflow-hidden rounded-[30px] bg-white/70 p-4">
          ...
          <div className="relative mx-auto flex min-h-[340px] max-w-[320px] items-center justify-center rounded-[28px] border border-white/70 bg-gradient-to-b from-[#FFF8F1] via-white to-[#ECF6FF] shadow-[0_18px_40px_rgba(31,82,152,0.15)] overflow-hidden">
            <div className="w-full h-full min-h-[320px]">
              <AvatarViewer modelUrl="/avatar.glb" isRecording={isRecording} />
            </div>
